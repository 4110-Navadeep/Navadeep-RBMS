...
const emailUser = (process.env.EMAIL_USER || 'donnaveeng@gmail.com').replace(/[<>]/g, '').trim();
const emailPass = (process.env.EMAIL_PASS || 'ytjh lwlj pqbn timj').replace(/[<>]/g, '').trim();
 ...
 * @param {object} bookingDetails - Booking details (resourceName, date, startTime, endTime, purpose, remarks)
...
const sendBookingEmail = async (toEmail, userName, bookingDetails, status) => {
    ...
    <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f3f4f6; margin: 0; padding: 20px;">
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); border: 1px solid #e5e7eb;">
        ...
        <div style="background-color: #0f2942; padding: 24px; text-align: center;">
          <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 600; letter-spacing: 0.5px;">Resource Booking Management System</h1>
        </div>
        ...
        <div style="padding: 32px 24px;">
          ...
          <h3 style="color: #374151; font-size: 16px; margin-top: 24px; border-bottom: 2px solid #f3f4f6; padding-bottom: 8px;">Booking Reservation Details</h3>
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 12px;">
            ...
            <tr>
              ...
              <td style="padding: 8px 0; font-size: 14px; font-weight: 600; color: ${statusColor};">
                <span style="display: inline-block; padding: 2px 8px; background-color: ${statusColor}15; border-radius: 4px;">
                  ${statusText}
                </span>
              </td>
            </tr>
  ...
  try {
    ...
    // Return null instead of crashing, allowing app to still function if SMTP is not configured/offline
    return null;
  }
};
