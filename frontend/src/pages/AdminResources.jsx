...
import { Plus, X, Layers, AlertCircle, CheckCircle2, IndianRupee } from 'lucide-react';
...
const AdminResources = () => {
  ...
  const handleOpenAddModal = () => {
    ...
    setImageUrl(''); setAvailability(true); setPrice('');
    setShowModal(true);
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this resource? All associated bookings will be deleted.')) return;
    setActionLoading(true);
    setErrorMsg(null); setSuccessMsg(null);
    try {
        ...
        <main className="p-6 md:p-8 space-y-6 max-w-7xl w-full mx-auto flex-1 flex flex-col">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-xl border border-slate-200 py-20 text-center shadow-sm flex-1 flex flex-col justify-center items-center">
              ...
              <button onClick={handleOpenAddModal} className="mt-4 px-4 py-2 bg-[#0f2942] text-white text-xs font-semibold rounded-lg">
                Create Resource
              </button>
            </div>
          ) : (
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-200">
            <form onSubmit={handleFormSubmit}>
              <div className="bg-[#0f2942] text-white px-6 py-4 flex items-center justify-between">
                <h3 className="font-bold text-lg">{editingResource ? 'Edit Resource Details' : 'Add New Resource'}</h3>
                <button type="button" onClick={() => setShowModal(false)} className="text-slate-300 hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Resource Name *</label>
                  <input type="text" required value={name} onChange={e => setName(e.target.value)}
                    placeholder="e.g. Premium Seminar Hall"
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0f2942] focus:bg-white transition-all" />
                </div>

                {/* Capacity + Base Price */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Max Capacity *</label>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <input type="number" value={price} onChange={e => setPrice(e.target.value)}
                        ...
                        className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0f2942] focus:bg-white transition-all" />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea rows="3" value={description} onChange={e => setDescription(e.target.value)}
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0f2942] focus:bg-white transition-all resize-none" />
                </div>

                {/* Availability */}
                <div className="flex items-center space-x-3 pt-1">
                  <input type="checkbox" id="availability" checked={availability}
                    onChange={e => setAvailability(e.target.checked)}
                    className="w-4 h-4 text-[#0f2942] border-slate-300 rounded focus:ring-[#0f2942] cursor-pointer" />
